...
% This is a MATLAB function that represents Adaptive Terminal Sliding Mode Control for a two-degree freedom robot.
...
% Adaption laws
b0dot = x0 * norm(S) * norm(inv(M));
b1dot = x1 * norm(S) * norm(inv(M)) * norm([x(1); x(2)]);
b2dot = x2 * norm(S) * norm(inv(M)) * norm([x(3); x(4)]);
dx = [qdot; q2dot; b0dot; b1dot; b2dot];
